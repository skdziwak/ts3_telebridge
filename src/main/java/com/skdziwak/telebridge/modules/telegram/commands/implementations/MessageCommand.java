...
@Command(name = "message", alias = {"msg", "m"}, permission = PermissionLevel.REGISTERED, requiresBridge = true, arguments = {"NAME", "MESSAGE"})
    ...
    public void command(CommandContext commandContext) {
        ...
        if (client.isPresent()) {
            ...
            if (message.length() > MAX_LENGTH) {
                ...
                ts3Api.sendPrivateMessage(client.get().getId(), message);
